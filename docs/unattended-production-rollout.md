...
| Phase 1b: Metamind attended deployment and proof | In progress | Pending | Production on baseline version `b7890ecc-b4c4-489f-a67f-3cafbca67b6a` via deployment `09d99cf6-6ef7-404b-81dd-99d36319ac77`; the unplanned version `998ca9c2-95cb-4731-a2a0-29bf94163acd` was rolled back at `2026-07-28T02:54:56Z`. See "Unplanned Phase 1b deployment" below | D1 lease migration APPLIED `2026-07-28T03:2xZ`: `wrangler d1 execute metamind --remote --file packages/worker/sql/add-browser-session-leases.sql` reported `changes: 1, changed_db: true, num_tables: 34`, and the verification query returns `browser_session_leases` with both indexes (`browser_session_leases_record`, `browser_session_leases_state`). Inert against the currently-deployed baseline, which does not reference the table. Still pending: the deploy itself, `/health.commit` verification, and the attended production proof | Pending | Release operator |
