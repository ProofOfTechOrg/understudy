...
The extension-ID file must contain the published 32-character Chrome ID. The
...
builds the local protocol package, and performs the dry run and deployment from
that immutable dependency snapshot. It rechecks the original tree before secret
upload and immediately before deployment. After
...
deployment, and any secret-derived active version separately.
...
Credential revocation, cloud-vault deletion, DNS changes, HSTS ramp changes, and deployment remain explicit operator actions. See the [production rollout](../../docs/unattended-production-rollout.md).
